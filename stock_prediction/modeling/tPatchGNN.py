...
class Model(nn.Module):
    ...
    def __init__(self, configs: ExpConfigs):
        ...
        # Make hidden dimension adaptive to input features, but ensure it's reasonable
        self.hid_dim = max(64, configs.enc_in + 16)  # At least 64, or enc_in + some extra
        ...
        if configs.task_name in ["short_term_forecast", "long_term_forecast", "imputation"]:
            ...
            # For classification, use a simpler decoder that outputs logits
            self.decoder = nn.Sequential(
                nn.Linear(enc_dim, self.hid_dim),
                nn.ReLU(inplace=True),
                nn.Linear(self.hid_dim, self.hid_dim),
                nn.ReLU(inplace=True),
                nn.Linear(self.hid_dim, 1)  # Single output for binary classification
            )
        else:
    ...
    def IMTS_Model(self, x, mask_X):
        ...
        # Calculate actual batch size from input tensor shape
        actual_batch_size = x_patch.size(0) // (self.ndim * self.n_patch)
        x_patch = x_patch.view(actual_batch_size, self.ndim, self.n_patch, -1) # (B, N, M, hid_dim)
        ...
        if(self.outlayer == "CNN"):
            x = x.reshape(actual_batch_size*self.ndim, self.n_patch, -1).permute(0, 2, 1) # (B*N, F, M)
            x = self.temporal_agg(x) # (B*N, F, M) -> (B*N, F, 1)
            x = x.view(actual_batch_size, self.ndim, -1) # (B, N, F)

        elif(self.outlayer == "Linear"):
            x = x.reshape(actual_batch_size, self.ndim, -1) # (B, N, M*F)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.configs.task_name in ["short_term_forecast", "long_term_forecast", "imputation"]:
            ...
            # For classification, we don't need time embeddings, just use the encoded features
            # h shape: (B, enc_in, hid_dim)
            # Take mean across features to get (B, hid_dim)
            h_pooled = h.mean(dim=1)  # (B, hid_dim)
            
            # Pass through classification decoder
            outputs = self.decoder(h_pooled)  # (B, 1)
            
            return {
                "pred": outputs.unsqueeze(1),  # (B, 1, 1) to match expected shape
                "true": y,
                "mask": y_mask
            }
        else:
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        # Handle dimension mismatch by adjusting positional encoding to match input
        batch_size, seq_len, feature_dim = x.shape
        if feature_dim != self.pe.size(-1):
            # If dimensions don't match, create a new positional encoding on the fly
            pe = torch.zeros(seq_len, feature_dim, device=x.device)
            position = torch.arange(0, seq_len, dtype=torch.float, device=x.device).unsqueeze(1)
            div_term = torch.exp(torch.arange(0, feature_dim, 2, device=x.device).float() * 
                               (-math.log(10000.0) / feature_dim))
            pe[:, 0::2] = torch.sin(position * div_term)
            if feature_dim > 1:
                pe[:, 1::2] = torch.cos(position * div_term[:pe[:, 1::2].size(1)])
            pe = pe.unsqueeze(0)  # (1, seq_len, feature_dim)
            x = x + pe
        else:
