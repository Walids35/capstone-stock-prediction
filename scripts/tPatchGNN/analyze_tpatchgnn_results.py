#!/usr/bin/env python3
"""
tPatchGNN Results Analysis Script
Analyzes TXT files from tPatchGNN output and calculates mean metrics across 5 tickers
...
TPATCHGNN_DIR = 'reports/output/tPatchGNN'

def parse_metrics_from_file(file_path):
    """
    Parse metrics from a TXT file.
    Returns the metrics dictionary from line 12 (tPatchGNN format).
    """
    try:
        ...
        if len(lines) >= 12:
            metrics_line = lines[11].strip()  # Line 12 (0-indexed)
...
def main():
    ...
    """
    # Create output file
    output_file = os.path.join(TPATCHGNN_DIR, "tpatchgnn_analysis_results.txt")
    ...
    try:
        ...
        print("=" * 80)
        print("tPatchGNN RESULTS ANALYSIS")
        print("=" * 80)
        print()
        
        # Check if tPatchGNN directory exists
        if not os.path.exists(TPATCHGNN_DIR):
            print(f"Error: Directory {TPATCHGNN_DIR} does not exist!")
            return
        ...
        # Find all TXT files
        txt_files = []
        for file in os.listdir(TPATCHGNN_DIR):
        ...
        for filename in txt_files:
            file_path = os.path.join(TPATCHGNN_DIR, filename)
