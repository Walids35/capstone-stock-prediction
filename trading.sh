...
for run in 'patchtst' 'tpatchgnn' 'timesnet' 'lstm'; do
    for model in 'finbert' 'roberta' 'deberta' 'lr' 'rf' 'svm'; do
        for ticker in 'AMZN' 'AAPL' 'TSLA' 'NFLX' 'MSFT'; do
            echo "Running trading simulation with ${run} for ${ticker} with ${model} model"
            ...
            python stock_prediction/trading_simulation.py \
                ...
                --model_path models/${run}_Binary_Price_${model}_model.pth \
                --ticker ${ticker} \
                --news_model ${model} \
                --batch_size 32 \
                --dropout 0.5 
